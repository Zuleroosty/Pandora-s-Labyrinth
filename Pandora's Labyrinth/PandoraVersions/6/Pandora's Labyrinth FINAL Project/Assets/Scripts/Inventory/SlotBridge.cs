...
public class SlotBridge : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (slotItem.GetComponent<InvItemHandler>().itemName.Contains("Ammo"))
        {
            slotItem.GetComponent<InvItemHandler>().itemQuantity = GameObject.Find("----PlayerObjectParent----").GetComponent<PlayerController>().ammo;
        }
    }
}
