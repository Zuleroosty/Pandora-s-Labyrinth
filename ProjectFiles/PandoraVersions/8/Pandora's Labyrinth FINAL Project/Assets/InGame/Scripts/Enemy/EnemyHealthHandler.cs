    ...
    private void ProjectileCollisionTest()
    {
        if (childID < childMax && childMax == projectileParent.transform.childCount)
        {
            ...
            if (projectileSprite.name.Contains("RadialBurst"))
            {
                ...
                {
                    if (GameObject.Find("RadialBurst(Clone)").GetComponent<RadialBurst>().bombExplode)
                    {
                        transform.position += (transform.position - GameObject.Find("RadialBurst(Clone)").transform.position);
                        if (!radialDamage)
                        {
