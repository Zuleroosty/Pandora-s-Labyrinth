...
public class CollisionManager : MonoBehaviour
{
    ...
    public enum collisionType {soft,hard}
    collisionType objectCollision;
    ...
    void Start()
    {
        playerObject = GameObject.Find("PlayerCollider");
        playerScript = playerObject.GetComponent<PlayerController>();
        playerSprite = playerObject.GetComponent<SpriteRenderer>();
        thisSprite = GetComponent<SpriteRenderer>();
        offSet = 0.09f;
        if (this.CompareTag("SoftCollision"))
        {
            enableCollision = true;
            objectCollision = collisionType.soft;
        }
        if (this.CompareTag("HardCollision"))
        {
            enableCollision = true;
            objectCollision = collisionType.hard;
        }
    ...
    void Update()
    {
        if (enableCollision)
        {
            ...
            if (playerObject.transform.position.x > thisSprite.bounds.max.x - offSet)
            {
                ...
                {
                    pushBack = new Vector3((playerScript.speed), 0);
                    print("rightCol");
                }
            }
            if (playerObject.transform.position.x < thisSprite.bounds.min.x + offSet)
            {
                ...
                {
                    pushBack = new Vector3((playerScript.speed) * -1, 0);
                    print("leftCol");
                }
            }
            if (playerObject.transform.position.y > thisSprite.bounds.max.y - offSet)
            {
                ...
                {
                    pushBack = new Vector3(0, (playerScript.speed));
                    print("upCol");
                }
            }
            if (playerObject.transform.position.y < thisSprite.bounds.min.y + offSet)
            {
                ...
                {
                    pushBack = new Vector3(0, (playerScript.speed) * -1);
                    print("downCol");
                }
            }
