    ...
    void Update()
    {
        ...
        else
        {
            ...
            else if (Input.GetAxis("RHorizontalCon") != 0 || Input.GetAxis("RVerticalCon") != 0)
            {
                ...
                if (Input.GetAxis("RHorizontalCon") > 0 && transform.localPosition.x > -18) transform.localPosition += new Vector3(Input.GetAxis("RHorizontalCon") * -0.8f, 0, 0);
                if (Input.GetAxis("RHorizontalCon") < 0 && transform.localPosition.x < 18) transform.localPosition += new Vector3(Input.GetAxis("RHorizontalCon") * -0.8f, 0, 0);
                if (Input.GetAxis("RVerticalCon") > 0 && transform.localPosition.y > -10f) transform.localPosition += new Vector3(0, Input.GetAxis("RVerticalCon") * -0.8f, 0);
                if (Input.GetAxis("RVerticalCon") < 0 && transform.localPosition.y < 10f) transform.localPosition += new Vector3(0, Input.GetAxis("RVerticalCon") * -0.8f, 0);
