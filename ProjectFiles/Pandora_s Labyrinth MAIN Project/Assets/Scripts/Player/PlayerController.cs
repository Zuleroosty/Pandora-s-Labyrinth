...
using System.Collections.Generic;
 ...
 public class PlayerController : MonoBehaviour
 {
     ...
     public int flashTimer, speedCost, speedRequiredLevel;
     ...
     // PLAYER STATS
     public float health, maxHealth, healAmount, healthCooldown, healthIncrease, stamina, maxStamina, staminaCooldown, boostTimer, potionCooldown, bombCooldown;
     ...
     public int totalBombs, maxBombs, totalMedkits, maxMedkits;
     ...
     void Update()
     {
         // REFRESH PLAYER STATS AT START OF LEVEL
         ...
         if (gameManagerScript.gameState == GameManager.state.InGame)
         {
             ...
             // RESET VELOCITY BEFORE DETECTED INPUT
         ...
         else if (inCombat) inCombat = false;
     }

     public void ResetPlayerStats() // FULL RESET OF PLAYER STATS - PRIMARILY ON DEATH
     {
         inCombat = false;
         speedIncrease = 0;
         healthIncrease = 0;
     ...
     public void StartNewLevel() // MOVE PLAYER BACK TO LABYRINTH START POSITION ON CONTINUE
     {
         transform.position = new Vector3(0, 0, 0);
     }

     public void TakeDamage(float damageAmount) // CALLED WHEN INFLICTING DAMAGE TO PLAYER
     {
         if (!ignoreDamage)
         {
             GetComponent<PlayerFXHandler>().PlayTakeDamage();
     ...
     private void RegenAndCooldowns()
     {
         ...
         // COOL DOWNS - MAINLY TO STOP DOUBLE USAGE IN GAME OR WHEN CHANGING STATES
