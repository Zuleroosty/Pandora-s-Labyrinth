...
public class PathController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (frameRate <= 0) frameRate = 3; // CONTROLS HOW OFTEN COLLISION AND CODE IS RUN - DEFAULT 5 FRAMES
        currentStage = 1;
    }
    ...
    void Update()
    {
        if (owningObject == null) Destroy(gameObject);
            ...
            else
            {
                ...
                if (!GetComponent<SpriteRenderer>().bounds.Intersects(targetObject.GetComponent<SpriteRenderer>().bounds)) // STOP RUNNING CODE (FOR EFFECIENCY)
                {
                    ...
                    switch (currentStage)
                    {
                        ...
                        case 2:
                            if (owningObject != null && owningObject.GetComponent<PathMaster>().pathLinks < 4) SpawnNextLink();
                            break;
                        case 3:
                            if (prevLink == null || nextLink == null) Destroy(gameObject);
                            break;
                    }
                }
            }
    ...
    void GetNewLocation() // 1
    {
        ...
        if (dCol == null && uCol == null && lCol == null && rCol == null)
        {
            ...
            // CHECK LEFT/RIGHT
            if (targetObject.transform.position.x > transform.position.x) newDirection.x = 1;
            else newDirection.x = -1;
            // CHECK UP/DOWN
            if (targetObject.transform.position.y > transform.position.y) newDirection.y = 1;
    ...
    {
        if (owningObject != null) owningObject.GetComponent<PathMaster>().pathLinks--;
    }
}
