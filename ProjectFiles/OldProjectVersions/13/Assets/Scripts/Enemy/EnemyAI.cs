...
public class EnemyAI : MonoBehaviour
{
    ...
    void Update()
    {
        if (GameObject.Find(">GameManager<").GetComponent<GameManager>().gameState == GameManager.state.InGame)
        {
            ...
            {
                if (!playerRadius.GetComponent<SpriteRenderer>().bounds.Intersects(GameObject.Find(">GameManager<").GetComponent<GameManager>().currentRoomParent.GetComponent<SpriteRenderer>().bounds))
                {
                    transform.position = new Vector3(GameObject.Find(">GameManager<").GetComponent<GameManager>().currentRoomParent.transform.position.x, GameObject.Find(">GameManager<").GetComponent<GameManager>().currentRoomParent.transform.position.y, transform.position.z);
                    Destroy(pathObject.GetComponent<PathMaster>().nextLink.gameObject);
                }
            }
    ...
    private void PlayRandomFX()
    {
        ...
        if (fxTimer >= fxRand)
        {
            ...
            fxTimer = 0;
            GetComponent<AudioSource>().volume = Random.Range(0.09f, 0.15f);
