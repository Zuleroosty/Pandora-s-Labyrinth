...
public class CollisionManager : MonoBehaviour
{
    ...
    void Update()
    {
        if (GameObject.Find("----PlayerObjectParent----") != playerObject)
        {
            playerObject = GameObject.Find("----PlayerObjectParent----");
            playerScript = playerObject.GetComponent<PlayerController>();
            playerSprite = GameObject.Find("PlayerSprite").GetComponent<SpriteRenderer>();
        ...
        if (GameObject.Find(">GameManager<").GetComponent<GameManager>().gameState == GameManager.state.InGame)
        {
            if (!disableCollision)
            {
                if (!this.name.Contains("Player"))
                {
                    ...
                    if (!this.name.Contains("Enemy") && !this.name.Contains("RadialBurst")) UpdateProjectileCollision();
                }
                if (this.name.Contains("Player") || this.name.Contains("RadialBurst")) UpdateEnemyCollision();
            }
        }
    }
    private void UpdatePlayerCollision()
    {
        ...
        if (thisSprite.bounds.Intersects(playerSprite.bounds))
        {
            ...
            {
                if (transform.localPosition.x < 0) playerObject.transform.position += new Vector3(1, 0, 0);
                else if (transform.localPosition.x > 0) playerObject.transform.position -= new Vector3(1, 0, 0);
                else if (transform.localPosition.y < 0) playerObject.transform.position += new Vector3(0, 1, 0);
                else if (transform.localPosition.y > 0) playerObject.transform.position -= new Vector3(0, 1, 0);
            }
            else playerObject.transform.position += pushBack;
        }
    }
    ...
    private void UpdateProjectileCollision()
    {
        ...
        if (projectileChildID < projectileChildMax && projectileChildMax == projectileParent.transform.childCount)
        {
            ...
            if (GetComponent<SpriteRenderer>().bounds.Intersects(projectileSprite.bounds) && !projectileSprite.name.Contains("RadialBurst"))
            {
